...
> Action Workflows][parser]. We recommend the use of Popper's own YAML 
> syntax introduced in this section._
...
A step can create, read, and modify environment variables. When you 
create a step in a workflow, you can define environment variables 
using the `env` attribute of a step. For example, you could set the 
variables `FIRST`, `MIDDLE`, and `LAST` using this:
...
> another runtime you would want Popper to support._
...
cannot be installed. In these cases, a step can execute directly on 
...
In the first step above, the `ls -la` command is executed on 
the root of the repository folder (the repository storing the 
`.yml` file). The second one shows how to execute a script. 
...
> _**NOTE**: The working directory (the value of `$PWD` when a command 
