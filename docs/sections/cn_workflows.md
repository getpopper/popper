...
| Name        | Description |
| :---------- | :-------------------- |
| `needs`     | Identifies actions that must complete successfully before <br>this action will be invoked. It can be a string or an array of strings.<br>In the example workflow above, `ACTION2` is executed after `ACTION1` is successfully completed. **Note:** When actions with a common `needs` dependency run in parallel and one action fails, the remaining actions are cancelled automatically. |
