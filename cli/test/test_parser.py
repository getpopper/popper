...
class TestWorkflow(unittest.TestCase):
    ...
    def test_new_workflow(self):
        self.assertIsInstance(Workflow.new(_wfile("a", "yml")), YMLWorkflow)
        self.assertIsInstance(Workflow.new(_wfile("a", "workflow")), HCLWorkflow)
...
"""
    ...
    def test_substitutions(self):
        ...
        self.assertDictEqual(
            wf.steps,
            {
                "a": {"uses": "sh", "args": ["ls"], "name": "a", "next": {"b"}},
                "b": {
                    "needs": ["a"],
                    "uses": "sh",
                    "args": ["ls"],
    ...
    uses = "sh"
        ...
        self.assertDictEqual(
            wf.wf_dict,
            {
                ...
                "steps": {"a": {"uses": "sh"}, "b": {"needs": "a", "uses": "sh"}},
            },
        )
...
class TestYMLWorkflow(unittest.TestCase):
    ...
    def test_load_file(self):
        ...
        self.assertDictEqual(
            wf.wf_dict, {"steps": {"a": {"uses": "sh"}, "b": {"uses": "sh"}}}
        )
        self.assertListEqual(wf.wf_list, [{"uses": "sh"}, {"uses": "sh"}])
        self.assertDictEqual(wf.id_map, {1: "a", 2: "b"})
