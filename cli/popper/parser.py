...
class Workflow(object):
    ...
    def new(
        ...
        substitutions=[],
        allow_loose=False,
        include_step_dependencies=False,
    ):
    ...
    def validate_workflow_block(self):
        ...
        workflow_block_cnt = len(self.wf_dict.get("workflow", dict()).items())
        ...
        if not pu.of_type(workflow_block["resolves"], ["str", "los"]):
            log.fail("[resolves] attribute must be a string or a list " "of strings.")
    ...
    def check_for_unreachable_steps(self, skip=None):
        ...
        if unreachable - skipped:
            if skip:
                log.fail(f'Unreachable step(s): {", ".join(unreachable-skipped)}.')
            else:
    ...
    def filter_step(wf, step, with_dependencies=False):
        ...
        if with_dependencies:
            ...
            for ra in required_steps:
                ...
                common_steps = filtered_steps.intersection(a_block.get("next", set()))
                if common_steps:
...
class YMLWorkflow(Workflow):
    ...
    def normalize(self):
        ...
        for a_name, a_block in self.steps.items():
            ...
            if a_block.get("secrets", None):
                a_block["secrets"] = Workflow.format_command(a_block["secrets"])
