...
class TestDroneTranslator(PopperTest):
    ...
    def test_translate_step(self):
        ...
        drone_step = dt._translate_step(popper_step, 1)
        self.assertEqual(
            drone_step,
            Box(
                {
    ...
    def test_translate_step_optional(self):
        ...
        popper_step = Box({"uses": "docker://byrnedo/alpine-curl:0.1.8",})
        drone_step = dt._translate_step(popper_step, 1)
        self.assertEqual(
            drone_step,
            Box(
                {
                    "name": "Step 1",  # "name" attribute is required and is generated by step index
                    "image": "byrnedo/alpine-curl:0.1.8",
                }
            ),
        )
